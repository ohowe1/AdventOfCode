...
class AocClient:
    ...
    def collect_day(self, year: int, day: int,
                    template_solution="scripts/template.py"):
        """
        Collect day puzzle data :
        - README.md (puzzle)
        - test_input.txt (examples))
        - real_input.txt
        - template solution
        """
        console.rule(f"[bold green]Collect Day {day} – {year}")

        dest_day = Path(f"year_{year}/day_{day}")
        ...
        # ---------------- Test Input ----------------
        test_input_path = dest_day / "test_input.in"
        self._extract_test_input(article, test_input_path)

        # ---------------- Real Input ----------------
        real_input_path = dest_day / "real_input.in"
    ...
    def run_solution(self, year: int, day: int, part: int, test: bool,
        ...
        sol_dir = Path(f"year_{year}/day_{day}")
